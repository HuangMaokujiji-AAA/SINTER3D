...
def align_slices_cast(
    adata_st_list,
    coor_key="spatial",
    plot=False,
    cast_output_path="./CAST_alignment",
    cast_gpu=None,
    cast_diff_step=5,
    cast_epochs=400,
    cast_iterations=500,
    cast_dist_penalty1=0,
    cast_bleeding=500,
    cast_d_list=None,
    cast_attention_params=None,
    cast_dist_penalty2=None,
    cast_alpha_basis_bs=None,
    cast_meshsize=None,
    cast_iterations_bs=None,
    cast_attention_params_bs=None,
    cast_mesh_weight=None,
    cast_alpha_basis=None,
):
    """Run CAST pairwise alignment on a list of AnnData slices."""
    import CAST

    if cast_d_list is None:
        cast_d_list = [3, 2, 1, 1/2, 1/3]
    if cast_attention_params is None:
        cast_attention_params = [None, 3, 1, 0]
    if cast_dist_penalty2 is None:
        cast_dist_penalty2 = [0]
    if cast_alpha_basis_bs is None:
        cast_alpha_basis_bs = [500]
    if cast_meshsize is None:
        cast_meshsize = [8]
    if cast_iterations_bs is None:
        cast_iterations_bs = [400]
    if cast_attention_params_bs is None:
        cast_attention_params_bs = [[None, 3, 1, 0]]
    if cast_mesh_weight is None:
        cast_mesh_weight = [None]
    if cast_alpha_basis is None:
        cast_alpha_basis = [1/1000, 1/1000, 1/50, 5, 5]

    print("Using CAST for non-rigid spatial alignment.")
    ...
    if cast_gpu is None:
        if torch.cuda.is_available() and torch.cuda.device_count() > 0:
            ...
            raise RuntimeError(
                "CAST requested GPU mode, but torch.cuda.is_available() is False. "
                "Please check CUDA_VISIBLE_DEVICES and whether PyTorch is CUDA-enabled."
            )

    print("CAST gpu =", cast_gpu)
    ...
    for i in range(len(adata_st_list) - 1):
        ref_sample = f"slice_{i}"
        query_sample = f"slice_{i + 1}"

        pair_output_path = os.path.join(cast_output_path, f"{query_sample}_to_{ref_sample}")
        os.makedirs(pair_output_path, exist_ok=True)

        coords_raw = {
            ref_sample: adata_st_list[i].obsm["spatial_aligned"].copy(),
            query_sample: adata_st_list[i + 1].obsm[coor_key].copy(),
        }

        exp_dict = {
            ref_sample: _to_dense(adata_st_list[i].X),
            query_sample: _to_dense(adata_st_list[i + 1].X),
        }

        graph_list = [query_sample, ref_sample]

        embed_dict = CAST.CAST_MARK(
            coords_raw_t=coords_raw,
            exp_dict_t=exp_dict,
            output_path_t=pair_output_path,
            task_name_t=f"{query_sample}_to_{ref_sample}",
            gpu_t=cast_gpu,
            epoch_t=cast_epochs,
            if_plot=plot,
        )
        ...
        params_dist = CAST.reg_params(
            ...
            gpu=cast_gpu,
            diff_step=cast_diff_step,
            iterations=cast_iterations,
            dist_penalty1=cast_dist_penalty1,
            bleeding=cast_bleeding,
            d_list=cast_d_list,
            attention_params=cast_attention_params,
            dist_penalty2=cast_dist_penalty2,
            alpha_basis_bs=cast_alpha_basis_bs,
            meshsize=cast_meshsize,
            iterations_bs=cast_iterations_bs,
            attention_params_bs=cast_attention_params_bs,
            mesh_weight=cast_mesh_weight,
        )

        params_dist.alpha_basis = torch.Tensor(cast_alpha_basis).reshape(5, 1).to(params_dist.device)
        ...
        adata_st_list[i + 1].obsm["spatial_aligned"] = np.asarray(coords_final[query_sample]).copy()
